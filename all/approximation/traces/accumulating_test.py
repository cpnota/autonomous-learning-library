...
class TestAccumulatingTraces(unittest.TestCase):
    ...
    def test_init(self):
        np.testing.assert_equal(self.traces(STATE), np.array([0, 0, 0]))

    def test_update_once(self):
        np.testing.assert_equal(self.traces(STATE), np.array([0, 0, 0]))
        self.traces.update(1, STATE, 1)
        np.testing.assert_allclose(
            self.traces(STATE), np.array([0, 0.6, 0]))

    def test_call_one(self):
        np.testing.assert_equal(self.traces(STATE), np.array([0, 0, 0]))
        self.traces.update(1, STATE, 1)
        np.testing.assert_approx_equal(self.traces(STATE, 1), 0.6)
    ...
    def test_update_multiple(self):
        ...
        np.testing.assert_allclose(
            self.traces(STATE), np.array([0, 1.5, 0]))
        self.traces.update(1, STATE, 2)
        np.testing.assert_allclose(
            self.traces(STATE), np.array([0, 1.95, 0.6]))
    ...
    def test_clears_traces_on_done(self):
        ...
        np.testing.assert_allclose(
            self.traces(STATE), np.array([0, 1.5, 0.6]))
