...
class TestAccumulatingTraces(unittest.TestCase):
  ...
  def test_update_once(self):
    ...
    np.testing.assert_allclose(self.traces.call(state), np.array([0, 0.6, 0]))
  ...
  def test_update_multiple(self):
    ...
    np.testing.assert_allclose(self.traces.call(state), np.array([0, 1.5, 0]))
    self.traces.update(1, state, 2)
    np.testing.assert_allclose(self.traces.call(state), np.array([0, 1.95, 0.6]))
  ...
  def test_clears_traces_on_done(self):
    ...
    np.testing.assert_allclose(self.traces.call(state), np.array([0, 1.5, 0.6]))
