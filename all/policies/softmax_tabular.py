...
class SoftmaxTabular(Policy):
    def __init__(self, learning_rate, state_space, action_space):
        ...
        self.weights = np.zeros((action_space.n, state_space.n))
    ...
    def update(self, error, state, action):
        self.weights[:, state] += self.learning_rate * error * self.state_gradient(state, action)

    def gradient(self, state, action):
        grad = np.zeros((self.weights.shape))
        neg_probabilities = self.state_gradient(state, action)
    ...
    def state_gradient(self, state, action):
        """
