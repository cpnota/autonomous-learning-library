...
class TestPresetBuilder(unittest.TestCase):
    def setUp(self):
        ...
        self.builder = PresetBuilder(self.name, self.default_hyperparameters, MockPreset)
    ...
    def test_call_api(self):
        agent = self.builder(device='cpu', hyperparameters={"lr": 0.01}, name="cool_name").env(Mock).build()
        self.assertEqual(agent.name, "cool_name")
        self.assertEqual(agent.hyperparameters, {**self.default_hyperparameters, "lr": 0.01})
        self.assertEqual(agent.device, 'cpu')
