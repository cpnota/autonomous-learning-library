...
class GeneralizedAdvantageBufferTest(unittest.TestCase):
    ...
    def test_simple(self):
        buffer = GeneralizedAdvantageBuffer(
            self.v, self.features, 2, 1, discount_factor=0.5, lam=0.5
        )
        actions = torch.ones((1))
        states = State.array(
            [State({"observation": torch.tensor([float(x)])}) for x in range(3)]
        )
        rewards = torch.tensor([1.0, 2, 4])
        ...
        tt.assert_almost_equal(
            values, torch.tensor([0.1826, -0.3476, -0.8777]), decimal=3
        )
    ...
    def test_parallel(self):
        buffer = GeneralizedAdvantageBuffer(
            self.v, self.features, 2, 2, discount_factor=0.5, lam=0.5
        )
        ...
        rewards = torch.tensor([[1.0, 1], [2, 1], [4, 1]])
        buffer.store(states[0], actions, rewards[0])
        buffer.store(states[1], actions, rewards[1])
        ...
        tt.assert_almost_equal(
            values,
            torch.tensor([[0.183, -1.408], [-0.348, -1.938], [-0.878, -2.468]]),
            decimal=3,
        )
        ...
        td_errors[1] = rewards[1] + 0.5 * values[2] - values[1]
        tt.assert_almost_equal(
            td_errors, torch.tensor([[0.6436, 1.439], [1.909, 1.704]]), decimal=3
        )
        ...
        advantages[1] = td_errors[1]
        tt.assert_almost_equal(
            advantages, torch.tensor([[1.121, 1.865], [1.909, 1.704]]), decimal=3
        )
